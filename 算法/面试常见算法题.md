...
### 给定一个数组如[1,2,3,4...], n= 10;在数组里找出两个数，使得a+b=10;a可以等于b,但a、b应有不同索引。
...
```js
  ...
  function getDateArr(strDate) {
    ...
    while (startStamp < endStamp) {
      ...
      dateArr.push(`${new Date(nextMonthStamp).getFullYear()}-${formatDigit(new Date(nextMonthStamp).getMonth() + 1)}`)
...
###  一维1-100乱序数组，修改其中某一个元素为-1，找出修改的位置以及修改的元素


```js
  let res = arr.reduce((prev, next, i) => {
    if (next !== -1) {
